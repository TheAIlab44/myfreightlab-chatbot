...
document.addEventListener("DOMContentLoaded", async () => {
  ...
  function renderFolderItem(folder) {
    ...
    el.addEventListener("click", e => {
      if (!e.target.classList.contains("menu-button")) openFolder(folder.id);
    });
    ...
    // drag & drop dans dossier
    el.addEventListener("dragover", e => { e.preventDefault(); el.classList.add("dragover"); });
    el.addEventListener("dragleave", () => el.classList.remove("dragover"));
    el.addEventListener("drop", e => {
      e.preventDefault(); el.classList.remove("dragover");
      const dragging = document.querySelector(".file-item.dragging"); if (!dragging) return;
  ...
  function renderFileItem(file) {
    ...
    btn.addEventListener("click", e => { e.stopPropagation(); closeMenus();
      const menu = document.createElement("div"); menu.className = "context-menu";
      const ren = document.createElement("div"); ren.textContent = "Renommer";
